...
static inline void gpu_errchk(gpuError_t result, const char *file, int32_t line) {
...
BenchResult pingpong_hindexed_bytes(int rank, int N, size_t whitespace_ratio, bool synch , int tag) {
    double* data_d;
    double* data_h;
    
    gpuHostMalloc((void**)&data_h, sizeof(double) * N);
    ...
    for (size_t iter = 0; iter < NITER; ++iter) {
        if (rank == 0) {
            if (synch) {
                ...
                MPI_Recv(data_d, 1, s_hindexed_bytes, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            } else {
            ...
            if (synch) {
                MPI_Recv(data_d, 1, s_hindexed_bytes, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    bool sane = true;
    if (rank == 0) {
        double* verify_h = (double*)malloc(sizeof(double) * N);
        gpu_ERRCHK(gpuMemcpy(verify_h, data_d, sizeof(double) * N, gpuMemcpyDefault));
        
        for (size_t i = 0; i < N; ++i) {
            if (verify_h[i] != data_h[i]) {
                sane = false;
                break;
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        fprintf(stderr, "%-15s | %-12s | %-12s | %-12s | %-12s | %-6s\n", 
                "Bytes", "Pancake FP32 (ms)", "Pancake FP16 (ms)", "Pancake FP8 (ms)", "Control (ms)", "VALIDATION");
        fprintf(stderr, "--------------------------------------------------------------------------------------\n");
    }

    for (int N = 1 << 5; N < (1 << 26); N = N << 1) {
        BenchResult res_pancake_fp32 = pingpong_hindexed_bytes(rank, N, 5, false, 0);
        BenchResult res_pancake_fp16 = pingpong_hindexed_bytes(rank, N, 5, false, 42);
        BenchResult res_pancake_fp8  = pingpong_hindexed_bytes(rank, N, 5, false, 43);
        BenchResult res_sync         = pingpong_hindexed_bytes(rank, N, 5, true, 0);

        if (rank == 0) {
            double moved_bytes = sizeof(double) * ((double)N) * (5.0 / 6.0); 
            bool all_sane = res_pancake_fp32.is_sane && 
                            res_pancake_fp16.is_sane && 
                            res_pancake_fp8.is_sane && 
                            res_sync.is_sane;

            fprintf(stderr, "%-15zu | %-12.4f | %-12.4f | %-12.4f | %-12.4f | %-6s\n", 
                    (size_t)moved_bytes, 
                    res_pancake_fp32.avg_time * 1000.0, 
                    res_pancake_fp16.avg_time * 1000.0, 
                    res_pancake_fp8.avg_time * 1000.0, 
                    res_sync.avg_time * 1000.0, 
                    all_sane ? "true" : "false");
        }
    }
